package com.artillexstudios.axapi.nms.v1_21_R7_paper.items.data;
    ...
    public DataComponent<CompoundTag> customData() {
        return new DataComponent<>() {
            ...
            public CompoundTag get(Object item) {
                ...
                return new com.artillexstudios.axapi.nms.v1_21_R7_paper.items.nbt.CompoundTag(itemStack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.of(new net.minecraft.nbt.CompoundTag())).copyTag());
            }
        };
    }
    ...
    public DataComponent<Key> itemModel() {
        return new DataComponent<>() {
            ...
            public void apply(Object item, Key key) {
                ...
                itemStack.set(DataComponents.ITEM_MODEL, Identifier.fromNamespaceAndPath(key.namespace(), key.value()));
            }

            @Override
            public Key get(Object item) {
                ItemStack itemStack = (ItemStack) item;
                Identifier resourceLocation = itemStack.get(DataComponents.ITEM_MODEL);
    ...
    public DataComponent<ProfileProperties> profile() {
        return new DataComponent<>() {
            ...
            public ProfileProperties get(Object item) {
                ...
                var profileProperties = new ProfileProperties(profile.partialProfile().id(), profile.name().orElse(""));
                profile.partialProfile().properties().forEach((k, v) -> profileProperties.put(k, new ProfileProperties.Property(v.name(), v.value(), v.signature())));
