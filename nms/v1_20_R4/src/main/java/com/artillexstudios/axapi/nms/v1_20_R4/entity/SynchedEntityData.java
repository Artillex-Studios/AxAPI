...
public class SynchedEntityData {
    ...
    private <T> net.minecraft.network.syncher.SynchedEntityData.DataItem<T> getItem(EntityDataAccessor<T> key) {
        return (net.minecraft.network.syncher.SynchedEntityData.DataItem) this.items.get(key.id());
    }
    ...
    public List<net.minecraft.network.syncher.SynchedEntityData.DataValue<?>> packForNameUpdate() {
        List<net.minecraft.network.syncher.SynchedEntityData.DataValue<?>> list = new ArrayList<>();
        for (net.minecraft.network.syncher.SynchedEntityData.DataItem<?> next : this.items.values()) {
            if (next.getAccessor().id() == EntityData.CUSTOM_NAME.id()) {
                list.add(next.value());
                break;
            }
        }
