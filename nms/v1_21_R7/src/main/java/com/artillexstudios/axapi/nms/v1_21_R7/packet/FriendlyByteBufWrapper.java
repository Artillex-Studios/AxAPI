package com.artillexstudios.axapi.nms.v1_21_R7.packet;

import com.artillexstudios.axapi.items.HashedStack;
import com.artillexstudios.axapi.nms.v1_21_R7.items.WrappedItemStack;
import com.artillexstudios.axapi.nms.v1_21_R7.items.nbt.CompoundTag;
...
public record FriendlyByteBufWrapper(RegistryFriendlyByteBuf buf) implements FriendlyByteBuf {
    ...
    public Key readResourceLocation() {
        Identifier resourceLocation = this.buf.readIdentifier();
        return Key.key(resourceLocation.getNamespace(), resourceLocation.getPath());
    }

    @Override
    public void writeResourceLocation(Key key) {
        this.buf.writeIdentifier(Identifier.tryBuild(key.namespace(), key.value()));
    }
    ...
    public HashedStack readHashedStack() {
        return new com.artillexstudios.axapi.nms.v1_21_R7.items.HashedStack(net.minecraft.network.HashedStack.STREAM_CODEC.decode(this.buf));
    }

    @Override
    public void writeHashedStack(HashedStack stack) {
        net.minecraft.network.HashedStack.STREAM_CODEC.encode(this.buf, ((com.artillexstudios.axapi.nms.v1_21_R7.items.HashedStack) stack).stack());
    }
