package com.artillexstudios.axapi.nms.v1_21_R4.utils;
...
public class BossBar implements com.artillexstudios.axapi.utils.BossBar {
    ...
    public BossBar(Component title, float progress, Color color, Style style, Flag... flags) {
        ...
        event = new BossEvent(uuid, ComponentSerializer.instance().toVanilla(title), BossEvent.BossBarColor.CODEC.decode(JavaOps.INSTANCE, color.getInternalName()).getOrThrow().getFirst(), BossEvent.BossBarOverlay.CODEC.decode(JavaOps.INSTANCE, style.getInternalName()).getOrThrow().getFirst()) {
        };
        updateAddPacket();
    }
    ...
    public void setStyle(Style style) {
        this.style = style;
        event.setOverlay(BossEvent.BossBarOverlay.CODEC.decode(JavaOps.INSTANCE, this.style.getInternalName()).getOrThrow().getFirst());
        updateAddPacket();
        updateStyle();
    }
    ...
    public void setColor(Color color) {
        this.color = color;
        event.setColor(BossEvent.BossBarColor.CODEC.decode(JavaOps.INSTANCE, this.color.getInternalName()).getOrThrow().getFirst());
        updateAddPacket();
        updateStyle();
    }
