package com.artillexstudios.axapi.nms.v1_20_R2.loot;
...
public class LootTable implements com.artillexstudios.axapi.loot.LootTable {
    ...
    public LootTable(Key key) {
        this.lootTable = MinecraftServer.getServer().getLootData().getLootTable(new ResourceLocation(key.namespace(), key.value()));
    }
        ...
        for (Map.Entry<LootContextParam<?>, Object> param : lootParams.params().entrySet()) {
            ...
            if (param.getValue() instanceof Player player) {
                if (param.getKey() == LootContextParams.DAMAGE_SOURCE) {
                    value = ((CraftPlayer) player).getHandle().damageSources().playerAttack(((CraftPlayer) player).getHandle());
                } else {
                    value = ((CraftPlayer) player).getHandle();
                }
            } else if (param.getValue() instanceof Entity entity) {
                if (param.getKey() == LootContextParams.DAMAGE_SOURCE) {
                    value = ((CraftEntity) entity).getHandle().damageSources().mobAttack(((CraftLivingEntity) entity).getHandle());
                } else {
                ...
                value = new Vec3(location.x(), location.y(), location.z());
            } else {
                value = null;
            }
